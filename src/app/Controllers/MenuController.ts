...
import { MenuInputModel } from 'app/Models/Menu';
...
class MenuController {
    ...
    public async datatables(request: Request, response: Response, next: NextFunction) {
        try {
            ...
            const data = await this.menuService.datatables(limit, offset, order, filters);
            const result = ResponseHelper.success({data : data})
            return response.status(200).json(result);
        } catch (error) {
            if (error instanceof BadRequestError) {
    ...
    public async findOne(request: Request, response: Response, next: NextFunction) {
        try {
            ...
            const data = await this.menuService.findOne(Number(id));
    ...
    public async create(request: Request, response: Response, next: NextFunction) {
        try {
            ...
            const params  = getParamsValidated(request, rules) as MenuInputModel;

            const data = await this.menuService.create(params);
    ...
    public async update(request: Request, response: Response, next: NextFunction) {
        try {
            ...
            const data = await this.menuService.update(Number(id), name);
    ...
    public async delete(request: Request, response: Response, next: NextFunction) {
        try {
            const id = request.params.id;
            const data = await this.menuService.delete(Number(id));
